# Abdulla Mehdi

Systems Architecture student at University of Borås, graduating June 2026. Based in Göteborg.

I build things with .NET and occasionally break things with Python. Currently interested in backend systems, ML applications, and figuring out how to make code that doesn't embarrass me six months later.

## What I've been working on

- **[Digital eCom Assistant]([link](https://abdullamehdi.com/projects/viskan-vea-digital-ecommerce-assistant))** - ML models for sales forecasting, built for Viskan Systems
- **[VIBE]([link](https://github.com/Abdriano95/VIBE-ConcertBookingApp))** - Fullstack concert booking app with .NET MAUI and ASP.NET Core Web API
- **[The Samaritan](https://abdriano.itch.io/the-samaritan)** - Unity game that won Borås Tech Challenge 2025 (Best Pitch + Best Student Contribution)
- **[MeTube]([link](https://github.com/Abdriano95/MeTube))** - YouTube-style streaming app with Azure Blob Storage

More projects: [abdullamehdi.com](https://abdullamehdi.com)

## Tech I actually use

**Most of the time:** C#, .NET, ASP.NET Core, Entity Framework, SQL Server, Python

**Often enough:** Docker, Azure, Git, React, scikit-learn

**Learning:** Kubernetes, CI/CD pipelines, ML beyond the basics

## Links

- [Portfolio](https://abdullamehdi.com)
- [LinkedIn](https://www.linkedin.com/in/abdulla-mehdi)
